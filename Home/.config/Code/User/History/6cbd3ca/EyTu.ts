namespace Constants {
	export class MeshRenderer {
		...
		Startup() {
			Kobold.requireComponent(Constants.Mesh);
			this.mesh = Kobold.getComponent(Constants.Mesh);
		...
		Render() {
			...
			if (this.mesh.meshType === Constants.MeshType.Circle) {
				this.context.beginPath();
				this.context.arc(translation.x, translation.y, scale.x, 0, Math.PI * 2);
				this.context.fill();
				return;
			}
			if (this.mesh.meshType === Constants.MeshType.Rect) {
