...
const compile = (filePath) => {
	...
	try { name = getNameFromPath(filePath, ".comp.ts") } catch (e) { Log(e); return; }
	Log(contents.outputText, `function Build() {${contents.outputText} return ${name};}`)
 ...
 * @param {String} ending
 */
const getNameFromPath = (path, ending) => {
	return path.split("/").at(-1).slice(0, -1 * ending.length);
}
