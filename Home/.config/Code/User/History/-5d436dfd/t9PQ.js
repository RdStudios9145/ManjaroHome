...
const open = (event, project) => {
	...
	const { builtInSliced, filesSliced } = JSON.parse(getAllNames(".const.ts"));
	const names = builtInSliced.concat(filesSliced);
	Log(consts);
	for (var i = 0; i < consts.length; i++) {
		console.log(consts[i]);
		Constants[names[i]] = consts[i];
	}
	Log(Constants);
}
...
const compile = (filePath) => {
	...
	try { eval(`function Build() {${contents.outputText} return Constants.${name};}`); }
		catch (e) { Log(e); return; }
...
/** @returns {{builtIn: String[], files: String[], builtInSliced: String[], filesSliced: String[]}} */
